    ...
    public void execute(String command, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (args[0].equalsIgnoreCase("servergroup")) {
                ...
                try (FileWriter fileWriter = new FileWriter("groups/" + group + ".json")){
                ...
                this.masterModule.getChannel().writeAndFlush(new Packet(PacketType.GROUP_LOADED_PACKET.name(), new GroupLoadedPacket(group, jsonObject.get("isFallBack").getAsBoolean(), jsonObject.get("version").equals(VersionUtil.WATERFALL.name()) ? true : false, Arrays.asList(insert.get("name").getAsString()))));
